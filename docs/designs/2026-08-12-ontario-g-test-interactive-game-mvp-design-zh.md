...
- 必须遵守: 引擎无 DOM/React 依赖；评分只基于模拟时钟和事件；所有随机数来自 seed。
- 禁止做: 每帧保存 action、用动画位置直接判分、显示官方分数/通过概率、考试模式即时提示答案。
- 不确定时先问: rubric 阈值会改变正确驾驶含义，或视觉抽象无法清楚表达 gap/车道关系。

**本阶段验证**:

- 自动化测试: 同 seed/action 重放一致；暂停不推进；prompt 只触发一次；危险 finding 去重；并入同时检查速度、空间和观察。
- 手工 / workflow 验证: 键盘、鼠标、触摸各完成一次；禁音仍有字幕；低动态仍能答题和复盘。
- 回归检查: 页面隐藏自动暂停；恢复不追帧；base path 资源正常。
- 失败 / 边界检查: 连点肩检、按键失焦未释放、拒绝所有 gap、内容加载失败、语音不可用。

**退出标准**:

- [ ] 5–8 分钟垂直切片可在桌面和手机完成，报告可准确说明至少一个普通错误和一个危险行为；作者验收道路/距离可读性。
...
- [ ] 完成 Exam 与 Practice 行为差异、危险事件“结束/继续练完”、短场景重做。
...
- [ ] attempt 持久化 content/engine version、seed、actions、findings 和 summary。

**Agent 执行约束**:

- 必须遵守: 每个结论包含时间/位置、情境、动作、预期和依据；内容变化提升 contentVersion。
- 禁止做: 将个人失败历史写入公开用户资料；默认启用遥测；为了完整路线编造道路事实。
- 不确定时先问: 路线证据冲突、官方规则不能支持 rubric、需要改变已发布 attempt schema。

**本阶段验证**:

- 自动化测试: 六类 rubric 正/反例、普通错误累计、危险单次终止、Top 3 排序、存储失败降级、旧 findings 可读。
- 手工 / workflow 验证: Chrome/Safari/Firefox 完整 20 分钟；手机触控；改键冲突；清除历史；导出 JSON。
- 回归检查: 不同考点/route ID 不混用；虽然 MVP 只有 Newmarket，也用 planned fixture 验证隔离。
- 失败 / 边界检查: IndexedDB 禁用/满额、刷新运行中 attempt、route version 不可重放、严重错误后继续。

**退出标准**:

- [ ] Newmarket 完整流程满足第 2 节成功标准；作者验收六类场景的教学表达和免责声明。
